...
| Placar | Admin manual | Automático via live polling ou WebSocket |
| Odds | `OddsCalculator` interno (Elo) | GameForecastAPI (importadas) |
| Tipos de mercado | Somente resultado (1X2) | Resultado, Over/Under, BTTS, Placar Exato, 1º Tempo |
...
> **Alternativa ao polling:** APIfootball oferece WebSocket em
> `wss://wss.apifootball.com/livescore?APIkey={key}` que envia push notifications de score
> e estatísticas sem consumir o limite de requisições. Recomendado para planos onde o
> limite diário é restrito. Implementação opcional na tarefa RF-13.
...
FIRST_HALF_RESULT   // Resultado do 1º tempo
```
...
- `FIRST_HALF_RESULT`: 3 outcomes fixos (mesmos do MATCH_RESULT)
...
- Para cada partida no response:
  ...
  - Atualiza `halftimeScore` quando disponível (útil para mercado `FIRST_HALF_RESULT`)
...
- **RF-14** (Opcional) Implementar conexão WebSocket com APIfootball como alternativa
  ao polling, ativada por flag em `application.properties`
- **RF-15** Lógica de cancelamento de evento: devolver stakes, mudar BetSlipItems para `CANCELLED`
- **RF-16** Propriedade `live-poll-enabled` para desligar scheduler em dev
...
|---|---|---|
| Cancelamento de evento | Devolver stake integral ou liquidar ao odd 1.0 | Devolver stake integral |
| Re-sync de odds | Manual (admin) vs automático 48h antes | Manual para começar |
| Bracket da Copa | Sortear oitavas automaticamente pós-grupos ou admin configura | Admin configura manualmente |
| Mercados ativos | Todos os 6 tipos ou subconjunto por torneio | Configurável via checklist na criação |
| Live score | Polling (mais simples) vs WebSocket APIfootball (sem custo de req) | Polling v1, WebSocket como melhoria futura (RF-14) |
| Match de fixtures (APIfootball ↔ GameForecast) | Nome + data vs mapeamento manual de IDs | Nome + data com fallback para log de aviso |
...
|---|---|
| **BUG-01 a BUG-08** (TournamentServiceImpl) | Bracket da Copa usa a mesma lógica de bracket FIFA. Corrigir antes de RF-09. |
| **Group 12 Deploy** (Dockerfile, `.env.example`) | As novas variáveis de API precisam ser incluídas. Coordenar. |
