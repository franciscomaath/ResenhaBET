...
> Last updated: June 13 2026 (Backend implementation complete)
...
❌ Standings table on Tournament Page
(use GET /api/v1/tournaments/{id}/scoreboard — endpoint exists)
...
## Group Bracket — Tournament Format Expansion 🔶 In Progress
...
Add fields with clear UI explanation for each option:
Format selector: LEAGUE / BRACKET / LEAGUE_BRACKET (with description of each)
Generation Mode: toggle "Generate automatically" / "Configure manually"
with explanation text for each option
Third Place Match: checkbox, visible only for BRACKET and LEAGUE_BRACKET
Number of Groups: number input, visible only for LEAGUE_BRACKET
populated dynamically from GET /api/v1/tournaments/group-config?playerCount={n}
after players are added — show valid options only
display player distribution preview (e.g. "2 groups of 4 players each")
Players Advancing Per Group: number input, visible only for LEAGUE_BRACKET
validation: total advancing must be >= 2

**B23. Bracket visualization component on Tournament Page**
Replaces standings placeholder when format=BRACKET.
Shown as second tab ("Bracket") when format=LEAGUE_BRACKET alongside the group standings tab.
Layout: columns = rounds, rows = match slots.
Each match card shows: playerHome name + team badge vs playerAway name + team badge,
score (if played), winner highlighted, penalties shown if applicable.
Empty slots show "TBD".
Bye events shown with "BYE" label.
Data from GET /api/v1/tournaments/{id}/rounds + GET /api/v1/events?tournamentId={id}.

**B24. Group stage standings on Tournament Page**
For LEAGUE_BRACKET: show per-group standings tables in the Matches tab or a dedicated Groups tab.
For LEAGUE: existing standings table using GET /api/v1/tournaments/{id}/scoreboard.

**B25. Penalties input on Event Page**
When event.isKnockout=true and admin clicks "End Match" with homeScore==awayScore:
Show penalty input fields (penaltiesHome, penaltiesAway) before confirming.
Both fields required, must be different — confirm button disabled otherwise.
Submit includes penalties in the finishEvent request body.

**B26. "Advance to Bracket" button on Tournament Page**
Admin only. Visible when format=LEAGUE_BRACKET, status=IN_PROGRESS,
and all GROUP_STAGE events are COMPLETED.
On click: confirmation dialog explaining that group stage is over and bracket will be generated.
Calls POST /api/v1/tournaments/{id}/advance-to-bracket.
On success: refresh tournament data, show bracket tab.

**B27. Group number display**
In the Players Tab of a LEAGUE_BRACKET tournament, show each player's group number.
In the Matches Tab, group events by round AND by group number during the group stage.
...
⏳ **Phase 7 (Frontend)** — Not started. UI updates (B22–B27).
