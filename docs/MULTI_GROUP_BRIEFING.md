...
User (userType: ADMIN | USER — system-wide)
 ...
 ├── Session
 │
 └── GroupMember (role: ADMIN | MEMBER — group-scoped) [proposed]
       │
       ▼
     Group
       │
...
**New field, this session:** `role` (proposed: `ADMIN` | `MEMBER`, not yet finalized). This role governs actions **within a single group's boundary only** — it has no authority over system-wide or cross-group operations. See Authorization Model below for the full rule and rationale.
...
`GroupMember.role` (proposed: `ADMIN` | `MEMBER`) is new and governs actions **scoped exclusively to one group** — it carries no authority outside that group's boundary, and specifically **does not** substitute for `userType == ADMIN` on system-wide actions.

A user can be a `groupRole == ADMIN` of Group A with full control over Group A's own affairs, and simultaneously have no more system-wide authority than any other `USER`.

## Confirmed rule

Everything that involves `Competition` — creation, market voiding, and result decisions on shared infrastructure — requires `userType == ADMIN`. A group-scoped admin role is explicitly **not** sufficient for these, regardless of which group the user administers.
...
* `FIFA_MATCH` tournaments have exactly one `GroupTournament` (one group). Any action on them — including entering a final score — is naturally contained to that one group. `groupRole == ADMIN` of that group is sufficient.
...
| Void a `CUSTOM_STAT` market on a `FIFA_MATCH` tournament | future void endpoint | `groupRole == ADMIN` of the owning group **(proposed — confirm)** |
| Decide result of a `FIFA_MATCH` event | `PATCH /api/v1/events/{id}/end` | `groupRole == ADMIN` of the owning group **(proposed — confirm)** |
| Manage group membership, `GroupTournament` settings | `POST /api/v1/groups/{id}/members`, future settings endpoint | `groupRole == ADMIN` |
| Switch active group context | `POST /api/v1/groups/{id}/switch` | Any `GroupMember` |

The two rows marked "(proposed — confirm)" follow logically from the stated principle but were not explicitly named in this session's decision — worth a quick confirmation before implementation.
...
Competition
Group
GroupMember (role: ADMIN | MEMBER) [proposed]
Team
...
* **Exact values for `GroupMember.role`.** Proposed `ADMIN | MEMBER`, not yet confirmed.
...
* **"Create vs Join" UX flow for a shared Tournament.** When Group B wants to participate in a `REAL_FOOTBALL` `Competition` that Group A already started, how does Group B discover the existing `Tournament` and attach a `GroupTournament` instead of accidentally creating a duplicate? No endpoint or UI flow defined yet.
...
* **The two "(proposed — confirm)" rows in the Authorization Model table** — `CUSTOM_STAT` market voiding and `FIFA_MATCH` result entry follow logically from the stated blast-radius principle but weren't explicitly confirmed this session.
